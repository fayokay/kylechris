 ...
 */
    ...
    if (docType === "documents") {
       // alert(docType);
        ...
        if (password === undefined) { password = ""; }
            ...
            //    var posting = $.post(documentsUrl, { "folder": $("input[name=folder]").val(), "status": $("input[name=status]:checked").val(), "type": $("input[name=type]:checked").val(), "extension": $("input[name=extension]:checked").val(), "password": password, "_token": $('meta[name="csrf-token"]').attr('content') });          
           // }
           // else{

                var posting = $.post(documentsUrl, { "folder": $("input[name=folder]").val(),  "search": $("input[name=search]").val(),  "status": $("input[name=status]:checked").val(), "type": $("input[name=type]:checked").val(), "extension": $("input[name=extension]:checked").val(), "password": password, "_token": $('meta[name="csrf-token"]').attr('content') });
            //}
        }else{
        var posting = $.post(templatesUrl, { "_token": $('meta[name="csrf-token"]').attr('content') });
    }
 ...
 function loadDocuments2(password){
 ...
 */
 function openFile(document_key){
    showLoader();    
    redirect(appUrl+"/document/open/"+document_key);
 ...
 */
 $(".folder-actions").on("click",  "a", function(event){
    ...
    if (action === "open") {
        ...
        var data =  { "folder": $(".signer-active-select").attr("data-id"), "_token": $('meta[name="csrf-token"]').attr('content') };
 ...
 */
 $(".file-actions").on("click",  "a", function(event){
    ...
    var action = $(this).attr("action");
    if (action === "open") {
        //alert($(".signer-active-select").attr("document_key"));
        ...
        var data =  { "file": $(".signer-active-select").attr("data-id"), "_token": $('meta[name="csrf-token"]').attr('content') };
 ...
 * tools bar actions
 */
 $(".user-social-detail").on("click",  "a", function(event){
    event.preventDefault();
    ...
    if (action === "open") {
        ...
        if ($(".ui-selected.folder").length > 1){
            ...
            deleteDocuments(deleteUrl, { "data": data, "_token": $('meta[name="csrf-token"]').attr('content') });
        }else{
            if ($(".ui-selected").attr("data-type") === "folder") {
                deleteDocuments(deleteFolderUrl, { "folder": $(".ui-selected").attr("data-id"), "_token": $('meta[name="csrf-token"]').attr('content') });
            }else if ($(".ui-selected").attr("data-type") === "file") {
                deleteDocuments(deleteFileUrl, { "file": $(".ui-selected").attr("data-id"), "_token": $('meta[name="csrf-token"]').attr('content') });
            }
        }
    }
 });
